...
def train_loop(dataloader, model, loss_fn, optimizer):
    ...
    for batch, (X, y) in enumerate(dataloader, start=1):  # enumerate()将一个可遍历的数据对象组合为一个索引序列，同时列出数据和数据下标
        ...
        if batch == 1 or batch % 100 == 0:
            ...
            print(f"loss: {loss:>7f}  [{current:>5d}/{size:>5d}]")  # > 表示右对齐（默认左对齐） f 表示浮点数 宽度7


def test_loop(dataloader, model, loss_fn):
    size = len(dataloader.dataset)
    num_batches = len(dataloader)  # 抓取的批次数
    ...
    test_loss /= num_batches  # 每轮的平均误差——用一个批次来表征
    correct /= size  # 每轮的正确率
