...
from config import VCR_IMAGES_DIR, VCR_ANNOTS_DIR, VCR_BERTS_DIR
...
def _fix_tokenization(tokenized_sent, bert_embs, old_det_to_new_ind, obj_to_type, token_indexers, pad_ind=-1):
    ...
    """
    最终得到类似结果: [('Does', 0), ('Riley', 1), ('feel', 0), ('comfortable', 0), ('?', 0)]
    """
    ...
    class BertField(SequenceField[Dict[str, torch.Tensor]])
        __init__(self, tokens: List[Token], embs: numpy.ndarray, padding_value: int = 0,
    ...
    """
    SequenceLabelField 为 text_field 中每个元素指定一个标签
    """
...
class VCR(Dataset):
    def __init__(self, split, mode, only_use_relevant_dets=True, add_image_as_a_box=True, embs_to_load='bert_da',
        ...
        :param conditioned_answer_choice: If you're in test mode, the answer labels aren't provided, which could be
                                          ...
                                          to always condition on the i-th answer. (正确答案标号)
        """
        self.split = split
        self.mode = mode
        self.only_use_relevant_dets = only_use_relevant_dets
        # flush 参数表示及时输出内容，不用等到文件关闭
        ...
        # VCR_ANNOTS_DIR = '/user-data/data-input/r2c/p2/data' <----from config.py import
        # split: train, val, or test
        # 读取split.jsonl的内容到items（列表，元素是字典）

        with open(os.path.join(VCR_ANNOTS_DIR, '{}.jsonl'.format(split)), 'r') as f:
            # items: List[Dict], 每一个item代表一张图片的信息（以字典的方式存储）212923个元素，每个元素有23个键
            self.items = [json.loads(s) for s in f]  # json.loads 把JSON文件转化为Python对象(字典)

        if split not in ('test', 'train', 'val'):
            raise ValueError("Split must be in test, train, or val. Supplied {}".format(split))

        if mode not in ('answer', 'rationale'):
            raise ValueError("Mode must be answer or rationale. Supplied {}".format(mode))

        # ELMoTokenCharactersIndexer()标记索引器
        # Convert a token to an array of character ids to compute ELMo representations.
        self.token_indexers = {'elmo': ELMoTokenCharactersIndexer()}
        self.vocab = Vocabulary()

        """
        获取所有的检测目标名称，作为coco_objects属性: List['__background__', 'name'] 一共81个
        编号，作为coco_obj_to_ind属性: Dict['name', ind]
        """
        # dirname(VCR_ANNOTS_DIR = '/user-data/data-input/r2c/p2/data') = '/user-data/data-input/r2c/p2'  <---- 一级目录名称
        # open /user-data/data-input/r2c/p2/dataloaders/cocoontology.json文件
        # 写入coco ----> 字典
        # coco['1']={'name': 'person', 'supercategory': 'person'}  即object属性值及其所属类别
        with open(os.path.join(os.path.dirname(VCR_ANNOTS_DIR), 'dataloaders', 'cocoontology.json'), 'r') as f:
            coco = json.load(f)
        # sorted(coco.items(), key=lambda x: int(x[0])) 将coco中的键值对按照键排序  ----> 列表
        # 得到['__background__', 所有object] ----> 列表
        self.coco_objects = ['__background__'] + [obj['name'] for num, obj in sorted(coco.items(), key=lambda t: int(t[0]))]  # 按编号排序
        # enumerate(self.coco_objects)返回一个列表，每个元素是一个元组(self.coco_objects的index, self.coco_objects)
        # self.coco_obj_to_ind 是一个字典，每个键值对是coco_objects: index
        self.coco_obj_to_ind = {o: i for i, o in enumerate(self.coco_objects)}

        # VCR_BERTS_DIR = '/user-data/data-input/r2c/p2/data/bert' <---- 三级目录
        # embs_to_load = 'bert_da'  传入的参数
        self.embs_to_load = embs_to_load
        # f前缀表示格式化字符
        self.h5fn = os.path.join(VCR_BERTS_DIR, f'{self.embs_to_load}_{self.mode}_{self.split}.h5')
        print("Loading embeddings from {0}".format(self.h5fn), flush=True)

    @property  # 训练数据集标志  ----> 可以改进，直接当作一个属性定义
    def is_train(self) -> bool:
        return self.split == 'train'

    """
    创建 train, val, test 数据集
    """
    @classmethod
    def splits(cls, **kwargs):
        ...
        # 个数可变的关键字形参，得到是一个字典
        # 字典的items()方法获取键—值对
        # kwargs = {'mode': 'answer', 'embs_to_load': 'bert_da', 'only_use_relevant_dets': True}
        kwargs_copy = {x: y for x, y in kwargs.items()}
        if 'mode' not in kwargs:
            kwargs_copy['mode'] = 'answer'
        # **kwargs_copy 表示字典参数传递
    ...
    def eval_splits(cls, **kwargs):
        ...
        stuff_to_return = [cls(split='test', mode='answer', **kwargs)] + [
            cls(split='test', mode='rationale', conditioned_answer_choice=i, **kwargs) for i in range(4)]  # 暂时理解为列表的扩张
        return tuple(stuff_to_return)

    def __len__(self):
        return len(self.items)  # items 是所有图片信息的列表集
    """
    返回选项和问题中所涉及到的对象下标dets2use，以及所有对象的一个新下标old_det_to_new_ind，涉及到都从1排序，未涉及的都为-1
    对于use all dets 的情形 dets2use 即为原 objects 的顺序，old_det_to_new_ind 在原有基础上 + 1
    """
    def _get_dets_to_use(self, item):
        ...
        answer_choices = item['{}_choices'.format(self.mode)]  # 问题的选项

        if self.only_use_relevant_dets:
            dets2use = np.zeros(len(item['objects']), dtype=bool)  # 一维数组，表示object个数
            people = np.array([x == 'person' for x in item['objects']], dtype=bool)  # object是person的标志数组
            for sent in answer_choices + [question]:  # 多增加一行字符（问题）
                for possibly_det_list in sent:  # 对应的回答里有检测到列表形式元素
                    if isinstance(possibly_det_list, list):
                        for tag in possibly_det_list:  # object的下标
                            if tag >= 0 and tag < len(item['objects']):  # 下标有效性检查
                                dets2use[tag] = True  # 对应下标的object在选项中将会用到
                    elif possibly_det_list.lower() in ('everyone', 'everyones'):  # 这种情况表示 object 中所有的人物都需要用到
                        dets2use |= people  # 位或运算，只要有一个为 True 即为True
            if not dets2use.any():  # 只要有一个True，条件即为假
                dets2use |= people
        else:
        ...
        old_det_to_new_ind[dets2use] = np.arange(dets2use.shape[0], dtype=np.int32)  # 使用到的objects的新下标
    ...
    def __getitem__(self, index):
        ...
        item = deepcopy(self.items[index])  # 深拷贝

        ###################################################################
        # Load questions and answers
        if self.mode == 'rationale':
            # 正确回答标号
            conditioned_label = item.get('answer_label') if self.split != 'test' else self.conditioned_answer_choice
            item['question'] += item.get('answer_choices')[conditioned_label]  # 正确回答加到question属性中

        answer_choices = item.get('{0}_choices'.format(self.mode))  # 所有回答
        ...
        """
        _fix_tokenization(tokenized_sent, bert_embs, old_det_to_new_ind, obj_to_type, token_indexers, pad_ind=-1)
            Return: (text_field, tags)
        """
        # zip(*[]): 先解包再打包
        ...
        ###################################################################
        # Load image now and rescale it. Might have to subtract the mean and whatever here too.

        # VCR_IMAGES_DIR = '.../data/vcr1images'
...
class VCRLoader(torch.utils.data.DataLoader):
    ...
    def from_dataset(cls, data, batch_size=3, num_workers=6, num_gpus=3, **kwargs):
        loader = cls(
            dataset=data,  # 传入的train、val、test
            batch_size=batch_size * num_gpus,  # 96 = 24*4
            shuffle=data.is_train,  # data.split == 'train'
            num_workers=num_workers,  # 7
            collate_fn=lambda x: collate_fn(x, to_gpu=False),  # lambda 是匿名函数，形参x，返回值是函数collate_fn(x, to_gpu=False)
